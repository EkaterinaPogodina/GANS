...
class DCGAN(GAN):
    ...
    def generator(self, z):
        with tf.variable_scope("generator") as scope:
            ...
            dense2 = tf.reshape(dense2, shape=(self.batch_size, 64 * self.seq_length))
            output = tf.layers.dense(dense2, units=self.seq_length)
    ...
    def discriminator(self, x, reuse=False):
        with tf.variable_scope("discriminator", reuse) as scope:
            ...
            dense2 = tf.layers.conv1d(dense1, filters=64, kernel_size=4, padding='same', activation='relu')
            dense2 = tf.reshape(dense2, shape=(self.batch_size, 64 * self.seq_length))
            logits = tf.layers.dense(dense2, units=1)
